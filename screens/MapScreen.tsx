...
import { LocationData, AppNotification } from '../types';
...
type HeatmapLayer = 'aqi' | 'pm25' | 'no2';

const MapScreen: React.FC<MapScreenProps> = ({ currentLocation, onLocationSelect, onAddNotification }) => {
  ...
  const [heatmapLayer, setHeatmapLayer] = useState<HeatmapLayer>('aqi');
  ...
  const generateHistory = (currentAqi: number, currentCarbon: number, baseDateStr: string, days: number = 7) => {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const isToday = i === 0;
      data.push({
        date: d.toLocaleDateString('en-US', { weekday: days > 10 ? undefined : 'short', month: 'short', day: 'numeric' }),
  ...
  const fetchWeather = async (lat: number, lng: number) => {
    ...
    try {
      const response = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current=temperature_2m,relative_humidity_2m,apparent_temperature,precipitation,weather_code,wind_speed_10m,wind_direction_10m&wind_speed_unit=ms`);
  ...
  useEffect(() => {
    ...
    mapRef.current.on('contextmenu', (e: any) => {
      ...
      onAddNotification({
        ...
        message: `New analysis point set at ${e.latlng.lat.toFixed(4)}, ${e.latlng.lng.toFixed(4)}`
      });
    });
    ...
    generateHistory(previewLocation.aqi, previewLocation.carbonIntensity || 450, historyBaseDate, historyDays);
  ...
  useEffect(() => {
    ...
    if (heatmapMode !== 'off') {
      generateHeatmap(heatmapMode === 'historical' ? heatmapDate : 'live', heatmapLayer);
    }
  }, [heatmapMode, heatmapDate, selectedCoords, heatmapLayer]);

  const generateHeatmap = (dateOrLive: string, layer: HeatmapLayer = 'aqi') => {
    if (!mapRef.current) return;
    ...
    const rnd = pseudoRandom(dateOrLive + center.lat + center.lng + layer);
    ...
    for(let i=0; i<12; i++) {
       ...
       let color = '#3b82f6';
       if (layer === 'aqi') {
         const randomVal = Math.floor(rnd() * 250);
         color = getAqiTheme(randomVal).color;
       } else if (layer === 'pm25') {
         const val = rnd() * 50;
         color = val < 12 ? '#10b981' : val < 35 ? '#f59e0b' : '#e11d48';
       } else {
         const val = rnd() * 40;
         color = val < 15 ? '#10b981' : val < 25 ? '#f59e0b' : '#e11d48';
       }
  ...
  const speakLocation = (name: string, aqi: number) => {
    ...
    const utterance = new SpeechSynthesisUtterance(`Currently analyzing ${name}. The air quality index is ${aqi}, which is considered ${getAqiTheme(aqi).label}.`);
  ...
  const updateLocation = (lat: number, lng: number, name?: string, shouldPan = false) => {
    ...
    setTimeout(() => {
      ...
      const locationName = name || `Coords: ${lat.toFixed(4)}, ${lng.toFixed(4)}`;
      ...
      generateHistory(mockAqi, mockCarbon, historyBaseDate, historyDays);
      speakLocation(locationName, mockAqi);
  ...
  useEffect(() => {
    if (debouncedSearchQuery && debouncedSearchQuery.length > 2 && isTyping) {
      handleSearch();
    ...
    const interval = setInterval(() => {
      ...
      if (rand > 0.8) {
        onAddNotification({
          ...
          message: `Sudden AQI spike detected in nearby ${previewLocation.name.split(',')[0]} area.`
        });
      } else if (rand > 0.6) {
        onAddNotification({
          type: 'info',
          title: 'Live Report',
          message: `Wind shift detected. Carbon intensity may fluctuate in the next hour.`
        });
      }
    }, 15000);

    return () => clearInterval(interval);
  }, [previewLocation.name, onAddNotification]);
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Locate precise coordinates and place details for: "${searchQuery}". 
                   Provide a very brief 1-sentence description of the area's environmental or geographic context.
                   Format: "LOCATION: [Name] | COORDS: [LAT, LNG] | SNIPPET: [Description]"`,
        config: { tools: [{ googleMaps: {} }] },
      });

      const coordMatch = response.text.match(/COORDS:\s*\[\s*(-?\d+\.?\d*)\s*,\s*(-?\d+\.?\d*)\s*\]/i);
      const snippetMatch = response.text.match(/SNIPPET:\s*([^|]+)/i);
      const snippet = snippetMatch ? snippetMatch[1].trim() : "Environmental monitoring area";
  ...
  const handleBaseDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    generateHistory(previewLocation.aqi, previewLocation.carbonIntensity || 450, newDate, historyDays);
  };

  const handleDaysChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const days = parseInt(e.target.value);
    setHistoryDays(days);
    generateHistory(previewLocation.aqi, previewLocation.carbonIntensity || 450, historyBaseDate, days);
  };
    ...
    <div className="relative h-screen w-full overflow-hidden">
      ...
      <div className="absolute top-24 right-6 z-[20] flex flex-col gap-3">
        ...
        <button 
          onClick={() => {
            if (window.speechSynthesis) {
              ...
              else speakLocation(previewLocation.name, previewLocation.aqi);
            }
          }}
          className={`size-12 rounded-2xl flex items-center justify-center shadow-2xl transition-all active:scale-90 border-2 ${isSpeaking ? 'bg-rose-500 text-white border-white' : 'bg-white text-slate-800 border-slate-100'}`}
        ...
        <button 
          ...
          className="size-12 rounded-2xl bg-primary text-white shadow-2xl border-2 border-white flex items-center justify-center active:scale-90 transition-all"
      ...
      {heatmapMode !== 'off' && (
        <div className="absolute top-24 left-6 z-[20] flex flex-col gap-2">
          <div className="bg-white/90 backdrop-blur-xl p-1.5 rounded-2xl shadow-xl border border-white flex flex-col gap-1">
            {(['aqi', 'pm25', 'no2'] as const).map((l) => (
              <button
                key={l}
                onClick={() => setHeatmapLayer(l)}
                className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${heatmapLayer === l ? 'bg-primary text-white shadow-md' : 'text-slate-400 hover:bg-slate-50'}`}
              >
                {l}
              </button>
            ))}
      ...
      <div className="absolute top-6 left-6 right-20 z-[20]">
        <form onSubmit={handleSearch} className="relative">
          <div className="flex items-center bg-white/95 backdrop-blur-2xl rounded-2xl border border-white shadow-2xl p-1.5 pr-4 transition-all focus-within:ring-4 focus-within:ring-primary/10">
            ...
            <input 
              type="text"
              className="flex-1 bg-transparent border-none focus:ring-0 text-sm font-bold text-slate-800 placeholder:text-slate-400 py-3 pr-10"
                ...
                <div key={i} className="flex flex-col border-b border-slate-50 last:border-0">
                  <button onClick={() => selectResult(result)} className="w-full px-5 py-3.5 flex items-start gap-4 hover:bg-slate-50 text-left transition-colors">
                    ...
                    <div className="flex flex-col min-w-0 flex-1">
                      ...
                      {result.snippet && (
                        <span className="text-[10px] text-slate-500 line-clamp-1 mt-0.5">{result.snippet}</span>
                      )}
                    </div>
                  </button>
                  {result.uri && (
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  <span 
                    className="material-symbols-outlined text-[18px] text-primary transition-transform duration-1000 ease-in-out"
                    style={{ transform: `rotate(${weatherData.windDirection}deg)` }}
                    title={`Direction: ${weatherData.windDirection}°`}
                  >
                    navigation
                  </span>
              ...
              <div className="mb-6 animate-in fade-in duration-500">
                <div className="flex flex-col gap-4 mb-6">
                  <div className="flex flex-col gap-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Base Date</label>
                    <input 
                      type="date" 
                  ...
                  <div className="flex flex-col gap-1">
                    <div className="flex justify-between items-center ml-1">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Analysis Range</label>
                      <span className="text-[10px] font-black text-primary">{historyDays} Days</span>
                    </div>
                    <input 
                      type="range"
                      min="3"
