...
const Calculator: React.FC<CalculatorProps> = ({ 
  ...
  useEffect(() => {
    if (debouncedSearchQuery && debouncedSearchQuery !== currentLocationName && isTyping) {
      handleSearch();
    }
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Find precise coordinates, carbon data, and air quality for: "${searchQuery}". 
                   Determine regional grid carbon intensity (gCO2/kWh) and current AQI.
                   Format: "LOCATION: [Name] | COORDS: [LAT, LNG] | INTENSITY: [Value] | AQI: [Value] | PROFILE: [Eco Level]"`,
        config: { tools: [{ googleMaps: {} }] },
      });

      const chunks = response.candidates?.[0]?.groundingMetadata?.groundingChunks || [];
      const co2Match = response.text.match(/INTENSITY:\s*(\d+)/i);
      const aqiMatch = response.text.match(/AQI:\s*(\d+)/i);
      const profileMatch = response.text.match(/PROFILE:\s*([^|]+)/i);
      const intensityValue = co2Match ? parseInt(co2Match[1]) / 1000 : 0.45;
      const aqiValue = aqiMatch ? parseInt(aqiMatch[1]) : 45;
      ...
      if (coordMatch && !showResults) {
        applyLocationContext(searchQuery, lat, lng, intensityValue, ecoLabel, aqiValue);
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const applyLocationContext = (name: string, lat: number, lng: number, co2Factor: number, profile: string, aqi: number = 45) => {
    setSearchQuery(name);
    onLocationChange?.({
      name,
      lat,
  ...
  const selectResult = (result: SearchResult) => {
    setShowResults(false);
    applyLocationContext(result.title, result.lat || 0, result.lng || 0, result.co2Factor || 0.45, result.ecoIntensity || "Regional Profile", result.aqi);
  };
                        ...
                        <div className="flex items-center gap-2">
                          {result.aqi && (
                            <div className="flex items-center gap-1 bg-blue-50 px-2 py-0.5 rounded-lg border border-blue-100">
                              <span className="material-symbols-outlined text-[10px] text-blue-500">air</span>
                              <span className="text-[9px] font-black text-blue-600 uppercase">AQI {result.aqi}</span>
                            </div>
                          )}
                          <div className="flex items-center gap-1 bg-emerald-50 px-2 py-0.5 rounded-lg border border-emerald-100">
                            <span className="material-symbols-outlined text-[10px] text-emerald-500">eco</span>
                            <span className="text-[9px] font-black text-emerald-600 uppercase">{result.ecoIntensity}</span>
                          </div>
                        </div>
                      </div>
                    </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm relative overflow-hidden group">
              <div className="flex flex-col gap-1">
                <p className="text-slate-400 text-[9px] font-black uppercase tracking-tighter">Grid Intensity</p>
                <div className="flex items-baseline gap-1">
                  <input 
                    className="w-24 border-none p-0 bg-transparent text-2xl font-black text-slate-800 focus:ring-0" 
                    placeholder="0.0" type="number" step="0.01"
                    value={data.co2Rate || ''}
                    onChange={e => setData(prev => ({ ...prev, co2Rate: parseFloat(e.target.value) || 0 }))}
                  />
                  <span className="text-[10px] font-bold text-slate-400 uppercase">kgCO₂/kWh</span>
                </div>
              </div>
                ...
                <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                  <div 
                    className={`h-full transition-all duration-1000 ${
                      data.co2Rate < 0.3 ? 'bg-emerald-500' : 
                      data.co2Rate < 0.6 ? 'bg-amber-500' : 'bg-rose-500'
                    }`}
                    style={{ width: `${Math.min(data.co2Rate * 100, 100)}%` }}
                  />
                </div>
                <span className={`text-[8px] font-black uppercase ${
                  data.co2Rate < 0.3 ? 'text-emerald-600' : 
                  data.co2Rate < 0.6 ? 'text-amber-600' : 'text-rose-600'
                }`}>
                  {data.co2Rate < 0.3 ? 'Clean' : data.co2Rate < 0.6 ? 'Moderate' : 'Heavy'}
                </span>
              </div>

              <div className="absolute top-0 right-0 p-3 opacity-0 group-hover:opacity-100 transition-opacity">
                <span className="material-symbols-outlined text-slate-300 text-sm cursor-help" title="Lower values indicate cleaner energy sources like wind or solar.">info</span>
              </div>
            </div>
